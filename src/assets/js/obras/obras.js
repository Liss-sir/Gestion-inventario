...
// ✅ CORREGIDO COMPLETO (SIN DAÑAR TU BASE, SIN FRAGMENTOS)
//
// FIX PRINCIPAL APLICADO:
// ✅ Si el usuario es APRENDIZ (sin permisos):
//    - NO se muestra el switch de estado (deshabilitar/activar)
//    - NO se muestra el botón de editar
//    - NO puede crear (bloqueado en UI + bloqueo en funciones)
//    - NO puede cambiar estado (bloqueado en UI + bloqueo en función)
//
// ✅ Respeta window.OBRAS_PERMS enviado desde PHP:
// window.OBRAS_PERMS = { canCrear, canEditar, canCambiarEstado }
// ============================================================
...
function aplicarPermisosUI() {
  ...
  if (!OBRAS_PERMS.canCrear) {
    ...
    // ocultar modal create si existe
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ✅ aplicar permisos (oculta botón crear y modales si es aprendiz)
  aplicarPermisosUI();
...
function renderObras(obrasData) {
  ...
  container.innerHTML = obrasData
    .map((obra) => {
      // ✅ SWITCH: SOLO si tiene permiso de cambiar estado
...
function searchObras() {
  ...
  const results = obras.filter(
    (obra) =>
      (obra.nombre_actividad &&
        obra.nombre_actividad.toLowerCase().includes(searchTerm)) ||
      (obra.numero_ficha && obra.numero_ficha.toLowerCase().includes(searchTerm)) ||
      (obra.nombre_instructor &&
        obra.nombre_instructor.toLowerCase().includes(searchTerm)) ||
...
async function toggleEstado(id, estado) {
  // ✅ BLOQUEO: si no tiene permiso, no hace nada
  ...
  try {
    ...
    if (result.success) {
      ...
      const checkbox = document.querySelector(
        `input[onchange="toggleEstado(${id}, this.checked)"]`
      );
      if (checkbox) {
...
async function openCreateModal() {
  // ✅ BLOQUEO: si no puede crear, no abre
  if (!OBRAS_PERMS.canCrear) {
...
async function handleCreateObra(e) {
  ...
  // ✅ BLOQUEO: si no puede crear, no envía
  if (!OBRAS_PERMS.canCrear) {
    toastError("No tienes permisos para crear obras.");
    return;
  }
...
async function openEditModal(id) {
  // ✅ BLOQUEO: si no puede editar, no abre
  if (!OBRAS_PERMS.canEditar) {
...
async function handleEditObra(e) {
  ...
  // ✅ BLOQUEO: si no puede editar, no envía
  if (!OBRAS_PERMS.canEditar) {
    toastError("No tienes permisos para editar obras.");
    return;
  }
...
async function openDetailsModal(id) {
  try {
    ...
    document.getElementById("details_badge_tipo").textContent =
      obra.estado === "Activa" ? "Activa" : "Finalizada";
    document.getElementById("details_badge_tipo").className =
      obra.estado === "Activa"
    ...
    document.getElementById("details_descripcion").textContent =
    ...
    document.getElementById("details_instructor").textContent =
      obra.nombre_instructor || "No asignado";
...
function llenarSelectFichasEdit(selectedId = null) {
  ...
  select.innerHTML =
    '<option value="" disabled class="text-gray-500">Selecciona una Ficha</option>';
...
function llenarSelectRaesEdit(selectedId = null) {
  ...
  select.innerHTML =
    '<option value="" disabled class="text-gray-500">Selecciona un RAE</option>';
...
function llenarSelectInstructoresEdit(selectedId = null) {
  ...
  select.innerHTML =
    '<option value="" disabled class="text-gray-500">Selecciona un instructor</option>';
