...
async function handleCreateObra(e) {
  ...
  if (!validarDescripcion(obraData.descripcion)) return;

  if (!validarFechas(obraData.fecha_inicio, obraData.fecha_fin)) return;
  ...
  try {
    ...
    if (result.success) {
      ...
      // Guardar obra creada (por si usas asignación)
...
async function cargarAprendicesParaSelect(idFicha) {
  try {
    ...
    try {
      ...
      select.innerHTML =
        '<option value="" disabled selected>Error al cargar aprendices</option>';
      return;
    }
    ...
    if (data && !data.error) {
      if (data.length === 0) {
        select.innerHTML =
          '<option value="" disabled selected>No hay aprendices en esta ficha</option>';
      } else {
        select.innerHTML =
