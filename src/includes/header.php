<?php
// ... (después de session_start y tus variables de usuario)
...
try {
    // 2. Obtener resumen (conteo)
    $stmtCount = $conn->prepare("SELECT COUNT(*) FROM notificaciones WHERE id_usuario = ? AND leida = 0");
    $stmtCount->execute([$id_usuario_logueado]);
    $noLeidas = $stmtCount->fetchColumn();

    // 3. Obtener las 5 más recientes
    $stmtNotif = $conn->prepare("SELECT * FROM notificaciones WHERE id_usuario = ? ORDER BY fecha_creacion DESC LIMIT 5");
    $stmtNotif->execute([$id_usuario_logueado]);
    $notificacionesRecientes = $stmtNotif->fetchAll(PDO::FETCH_ASSOC);
} catch (Exception $e) {
    $noLeidas = 0;
